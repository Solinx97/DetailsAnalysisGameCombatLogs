...
public class CommunityPostController(IMediator mediator) : ControllerBase
{
    ...
    {
        var command = new CreateCommunityPostCommand(request.CommunityName, request.Owner, request.Content, request.PostType, request.PublicType, request.Restrictions, request.Tags, request.AppUserId);
