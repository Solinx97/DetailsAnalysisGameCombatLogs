...
internal class GetPreAurasHandler(ICombatAbilityRepository repository, IMapper mapper) : IRequestHandler<GetPreAurasQuery, IEnumerable<UnitPreAuraDto>>
{
    ...
    {
        var combatPreAuras = await _repository.GetByPreAuraAsync(request.CombatId, cancellationToken);
        var map = _mapper.Map<IEnumerable<UnitPreAuraDto>>(combatPreAuras);
