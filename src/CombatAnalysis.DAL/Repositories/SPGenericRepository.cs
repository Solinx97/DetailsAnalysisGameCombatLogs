...
namespace CombatAnalysis.DAL.Repositories
{
        ...
        where TModel : class
    {
        ...
        async Task<IEnumerable<TModel>> ISPGenericRepository<TModel>.ExecuteStoredProcedureUseModelAsync(string procedureName, string[] paramNames, object[] paramValues)
        {
        ...
        async Task<int> ISPGenericRepository<TModel>.ExecuteStoredProcedureAsync(string procedureName, string[] paramNames, object[] paramValues)
        {
            var procedureParams = new List<SqlParameter>();
            var procedureParamNames = new StringBuilder();
            for (int i = 0; i < paramValues.Length; i++)
